...
func GetImage(path string, base64ImageContent string) ([]byte, string,error) {
	...
	allData := pre.FindAllSubmatch([]byte(base64ImageContent), 2)
	fileType := string(allData[0][1])                         //png ，jpeg 后缀获取
	base64Str := pre.ReplaceAllString(base64ImageContent, "") // 去掉前缀
